...
사용자가 아직 구현 전인 SW의 의도·범위를 좁히려는 경우에도 concept 모드를 사용한다. 이때 claim은 정답이 아니라 **현재의 제품 가설**이며 `contested`로 등록한다. 처음부터 긴 인터뷰를 하지 말고 다음 리듬을 지킨다.
...
3. 가능하면 서로 겹치지 않는 2~4개 선택지를 `--options`로 제시하고 마지막에 `아직 모르겠음`을 둔다. TUI에서는 클릭이나 방향키+Enter로 항목을 선택한 뒤 별도의 `답변 제출` 버튼으로 확정한다. 숫자 키도 선택만 하며 즉시 제출하지 않는다.
...
**Codex에서는 이 질문 루프 전체를 한 턴 안에서 계속 실행한다.** `squiz wait`가 실행 세션 ID를 반환하면 같은 실행 세션을 poll하여 답을 받을 때까지 기다리고, 답이 도착한 뒤에는 사용자에게 "재개"를 요청하거나 턴을 끝내지 않는다. 곧바로 판정하고 다음 `ask`와 `wait`를 실행한다. 도구의 단일 대기 시간이 제한되어 있으면 짧은 timeout으로 새 질문을 만들지 말고 **동일한 waiter를 반복 poll**한다. 사용자 입력이 필요한 곳은 별도 터미널의 TUI뿐이며, Codex 채팅에는 최종 요약이나 실제로 중단된 경우에만 응답한다.
