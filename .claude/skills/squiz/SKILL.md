---
name: squiz
description: 소크라테스식 이해 점검 세션을 진행한다. 사용자가 방금 구현된 코드, 일반 개념, 사람이 작성한 코드를 정말 이해했는지 확인하고 싶을 때 사용한다. "이해했는지 확인", "퀴즈", "squiz", "소크라테스식 점검" 요청 시 발동. 정답 맞추기가 아니라 예측·근거·경계·전이를 사용자 스스로 수행하게 이끈다.
---
...
사용자가 아직 구현 전인 SW의 의도·범위를 좁히려는 경우에는 `squiz init --source concept --purpose ideation`으로 시작한다. 종료 보고는 학습 정오 평가 대신 제품 결정·미결 사항을 안내한다. 이때 claim은 정답이 아니라 **현재의 제품 가설**이며 `contested`로 등록한다. 처음부터 긴 인터뷰를 하지 말고 다음 리듬을 지킨다.

1. 사용자의 말을 한 문단으로 반영하고, 아직 결정되지 않은 축을 최대 3개만 제시한다. 시작할 때 `빠른 수렴,균형,하나씩 깊게`로 진행 속도를 고르게 한다.
2. **화면에는 항상 질문 하나만 둔다.** 여러 질문을 한 화면에 나열해 한 답변으로 받지 않는다. 독립적인 사실 확인은 선택형 질문을 연속으로 빠르게 보내고, 앞 답에 따라 달라지는 질문은 반드시 답을 받은 뒤 만든다.
3. 가능하면 서로 겹치지 않는 2~4개 선택지를 `--options`로 제시하고 마지막에 `아직 모르겠음`을 둔다. TUI에서는 방향키와 Enter 또는 숫자 하나로 답할 수 있다. 선택지 밖 의견은 `i` → 자유 입력 → `Ctrl+D`로 제출한다(`Esc`로 선택지 복귀). 이 답은 정오 판정하지 말고 다음 후보와 요약에 반영한다. 부담 한도의 계속/요약/설명 화면에는 이 입력 경로가 없다.
4. 선택 결과를 다음 질문의 `--notice`에 한 줄로 누적해 사용자가 현재 구상을 계속 볼 수 있게 한다.
5. 선택이 실제로 구현 범위를 가르는 경우에만 후속 질문을 한다. 이미 답한 내용을 표현만 바꿔 재확인하지 않는다.
6. 자유 서술은 선택지로 포착할 수 없는 trade-off, 예외, 성공 기준에만 요청한다. 사용자가 `아직 모르겠음`을 고르면 정답을 요구하지 말고 권장 기본값과 그 비용을 제시한 뒤 채택/보류만 묻는다.
7. 3~5번의 응답마다 지금까지의 **목표 / 비목표 / 사용자 / 핵심 흐름 / 열린 결정**을 요약하고, `계속 좁히기,이 정도로 명세 초안 만들기,종료` 선택지를 준다.

속도 선택은 질문 수가 아니라 후속 탐색의 깊이를 바꾼다. `빠른 수렴`은 독립 축을 선택형으로 연속 확인하고 요약에서 충돌만 되짚는다. `균형`은 범위를 크게 바꾸는 답에만 후속 질문 하나를 한다. `하나씩 깊게`는 한 축의 성공 기준과 예외까지 합의한 뒤 다음 축으로 이동한다. 어느 모드에서도 compound question을 만들지 않는다.

선택지를 많이 쓰는 것은 사용자의 의도를 대신 정하는 것이 아니다. 선택지는 반드시 `직접 입력` 없이도 보류 가능한 경로를 포함하고, 사용자가 제시한 새 방향은 다음 질문의 후보와 최종 요약에 보존한다.
...
```bash
squiz init --source ai|concept|code [--role author|reviewer] # --role은 code에서만 사용
```
...
사용자는 **같은 작업 디렉터리**의 별도 터미널에서 `squiz ui`를 연다(안내하라). init 전이면 초기화를 기다리고, 새 세션에도 자동 연결된다. 설명과 `feedback --text`, `ask --notice`는 UI에 표시되고 재접속 후에도 복원된다. 긴 내용은 `PgUp/PgDn`으로 읽는다. `/skip` 또는 선택형 `s`는 질문 하나가 아니라 현재 개념의 남은 점검 전체를 미룬다. `/abort`는 세션 중단, `Ctrl+C`는 UI만 종료한다. 너는:
...
**wait exit 코드**: 0=답변(stdout JSON, 선택지 밖 의견은 action=proposal) / 2=clarify(재표현→2회째 양식 전환) / 4=중단 / 6=설명 요청(`explanation record`) / 7=이의(`objection --discard` 또는 `objection --reason "이유"`) / 8=skip.
