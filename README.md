...
두 개의 터미널을 **같은 작업 디렉터리**에서 엽니다. `squiz ui`는 init 전에도 종료하지 않고 기다리며, AI가 세션을 만들면 자동 연결됩니다. 이후 새 세션도 자동으로 따라갑니다.
...
상태는 `./.squiz/`에 저장됩니다(append-only 이벤트 로그가 원천). 안내문·설명·피드백은 재접속 후에도 표시되고, 완료·중단·CLI 처리 오류는 화면에 다음 행동과 함께 나타납니다. 긴 내용은 `PgUp/PgDn`으로 읽을 수 있습니다.

### SW 구상 좁히기

구현 전 아이디어는 `squiz init --source concept --purpose ideation`으로 시작합니다. 종료 보고는 제품 결정·미결 사항을 안내하며 학습 성취 평가를 표시하지 않습니다. 기존 세션과 기본 `--purpose learning`은 이해 점검 보고를 유지합니다. 화면에는 계속 **질문 하나만** 표시합니다. 여러 질문을 한꺼번에 받으면 답을 빠뜨리기 쉽고 앞 답에 맞춰 다음 질문을 바꿀 수 없기 때문입니다. 대신 시작할 때 진행 속도를 고릅니다.
...
즉 **한 화면 한 질문은 유지하되, 모든 질문을 깊게 파지는 않는 혼합 방식**입니다. 가능한 질문은 TUI 선택지로 바꾸며, **↑/↓ 또는 j/k + Enter**로 고르거나 **숫자 키 하나**로 즉시 답할 수 있습니다. 선택지 밖 의견은 `i`를 누르고 자유 입력한 뒤 `Ctrl+D`로 보냅니다(`Esc`로 선택지 복귀). 부담 한도의 계속/요약/설명 선택 화면은 정해진 선택지를 사용합니다. 몇 번의 선택마다 목표·비목표·사용자·핵심 흐름·열린 결정을 요약하고, 계속할지 명세 초안을 만들지 사용자가 고릅니다.

자유 서술이 필요한 질문에서는 기존처럼 여러 줄을 적고 `Ctrl+D`로 제출할 수 있으며, `/clarify`, `/explain`, `/object`로 도움이나 이의를 요청할 수 있습니다. `/skip`(선택형 `s`)은 **현재 개념의 남은 점검 전체를 미루고** 다음 개념 또는 마무리로 넘어갑니다. `/abort`(선택형 `a`)는 세션을 중단하며, `Ctrl+C`는 UI만 닫아 나중에 재접속할 수 있습니다.
